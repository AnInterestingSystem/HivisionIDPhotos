...
class IDPhotoProcessor:
    def process(
        ...
        render_option_index = LOCALES["render_mode"][language]["choices"].index(render_option)
        # 读取插件选项
        # 人脸对齐选项
    ...
    def _process_size_mode(self, idphoto_json, language, size_list_option, custom_size_height, custom_size_width, custom_size_height_mm, custom_size_width_mm, ):
        ...
        if idphoto_json["size_mode"] == LOCALES["size_mode"][language]["choices"][0]:
            idphoto_json["size"] = LOCALES["size_list"][language]["develop"][size_list_option]
        # 如果选择了自定义尺寸(px或mm)
        elif idphoto_json["size_mode"] == LOCALES["size_mode"][language]["choices"][2] or idphoto_json["size_mode"] == LOCALES["size_mode"][language]["choices"][3]:
            # 如果选择了自定义尺寸(px)
            if idphoto_json["size_mode"] == LOCALES["size_mode"][language]["choices"][2]:
    ...
    def _process_color_mode(idphoto_json, language, color_option, custom_color_r, custom_color_g, custom_color_b, custom_color_hex_value, ):
        ...
        # 如果选择了自定义颜色HEX
            ...
            if len(hex_color) == 6:
                idphoto_json["color_bgr"] = tuple(int(hex_color[i: i + 2], 16) for i in (0, 2, 4))
    ...
    def _process_generated_photo(self, result, idphoto_json, language, watermark_option, watermark_text, watermark_text_size, watermark_text_opacity, watermark_text_angle, watermark_text_space, watermark_text_color, ):
        ...
        # 渲染背景
        result_image_standard, result_image_hd = self._render_background(result_image_standard, result_image_hd, idphoto_json, language)
    ...
    # 渲染背景
    @staticmethod
    def _render_background(result_image_standard, result_image_hd, idphoto_json, language):
        """渲染背景"""
    ...
    # 生成模板照片
    @staticmethod
    def _generate_image_template(idphoto_json, result_image_hd, language, ):
        # 如果选择了只换底，则不生成模板照片
        if idphoto_json["size_mode"] in LOCALES["size_mode"][language]["choices"][1]:
            return None, False
    ...
    def _add_watermark(result_image_standard, result_image_hd, watermark_text, watermark_text_size, watermark_text_opacity, watermark_text_angle, watermark_text_space, watermark_text_color, ):
        ...
        result_image_standard = add_watermark(image=result_image_standard, **watermark_params)
    ...
    def _save_image(result_image_standard, result_image_hd, result_image_layout, idphoto_json, format="png", ):
        ...
        if custom_kb and custom_dpi:
            ...
            if result_image_layout is not None:
                save_image_dpi_to_bytes(result_image_layout, output_paths["layout"]["path"], dpi=custom_dpi)
        ...
        # 只有自定义KB的情况
            ...
            if result_image_layout is not None:
                save_image_dpi_to_bytes(result_image_layout, output_paths["layout"]["path"], dpi=300)
        ...
        # 没有自定义设置
            ...
            # 保存所有图像
            save_image_dpi_to_bytes(result_image_standard, output_paths["standard"]["path"], dpi=300)
            save_image_dpi_to_bytes(result_image_hd, output_paths["hd"]["path"], dpi=300)
            if result_image_layout is not None:
                save_image_dpi_to_bytes(result_image_layout, output_paths["layout"]["path"], dpi=300)
